...
export default function ServicesPage() {
  return <main><Header />
    ...
    <section className="detail-services">{services.map((s,index)=><article key={s.n}><div className="detail-photo"><Image src={`${assetBase}/gallery/praca-${String([3,11,14,20][index]).padStart(2,"0")}.webp`} alt={s.title} fill sizes="(max-width:800px) 100vw, 45vw" /></div><div><span>{s.n}</span><h2>{s.title}</h2><p>{s.text}</p>{index===3?<Link className="text-link" href="/poistne-udalosti">Ako postupovať pri škode →</Link>:<a className="text-link" href={site.phoneHref}>Dohodnúť obhliadku →</a>}</div></article>)}</section>
